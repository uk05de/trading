...
def render_position_trades_table(trades: list[dict], kp: str = ""):
    ...
    if _n > 1:
        ...
        st.dataframe(_style_trade_detail_df(_sum_df), hide_index=True, use_container_width=True, column_config=_get_trade_col_config(_is_closed))

    # Einzeltrades
    if _n > 1:
        st.markdown("**Einzelne Trades:**")
    df = pd.DataFrame(rows)
    st.dataframe(_style_trade_detail_df(df), hide_index=True, use_container_width=True, column_config=_get_trade_col_config(_is_closed))
