...
def calc_profit_r(trade: dict) -> float | None:
    """Profit in R-Vielfachen fuer einen offenen Trade."""
    ko = trade.get("ko_level")
    bv = trade.get("bv") or 1.0
    d = trade["direction"]
    entry_stock = trade["entry_price"]
    sl = trade.get("stop_loss")
    bid = trade.get("product_bid")

    if not (sl and ko and entry_stock):
        return None

    ep = stock_to_product(entry_stock, ko, d, bv)
    sp = stock_to_product(sl, ko, d, bv)
    cp = bid if bid else stock_to_product(
        trade.get("current_price") or entry_stock, ko, d, bv)
    risk = abs(ep - sp)
    if risk <= 0:
        return None
    return round((cp - ep) / risk, 2)
